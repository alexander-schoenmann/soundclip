...
//neue View anlegen
export let view = new KWM_View("/podcastList", async function(){

    if(window.Core.utils.isEmpty(KWM_View.getGetParameters().id)){
        window.location.hash = "/podcasts";
    }
    else{
        this.podcastList = await window.Core.model.getPodcastDetail(KWM_View.getGetParameters().id);
        if(this.podcastList == false){

        }
        else{
            await this.rendering(this.podcastList.id);
            ...
            new_comment.addEventListener("click", function () {
                ...
                window.Core.model.addCommentToPost(that.podcastList.id, comment);
            });
...
view.rendering = async function(id){
    ...
    await this.podcastList.renderSingleMarkup(document.getElementById("kwm-body")); //await geht nur wenn function async ist -> wartet auf ergebnis (promise) hinter await
